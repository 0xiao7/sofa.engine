...
test('airy visual direction replaces the old dark game lobby first impression', () => {
  ...
  assert.match(html, /AIRY_REFERENCE_SCENES/);
  assert.match(html, /assets\/room\/preview-library\.png/);
  assert.match(html, /assets\/room\/preview-island\.png/);
  assert.match(html, /function\s+_drawAiryReferenceScene/);
  assert.match(html, /function\s+_airyDrawCroppedCover/);
  assert.match(html, /function\s+_startAiryRoom/);
  assert.match(html, /body\.airy-room \.scene-mode-toggle\{display:flex;\}/);
  assert.match(html, /body\.airy-room/);
  assert.match(html, /Endless Minimal Library Walk/);

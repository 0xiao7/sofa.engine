...
test('login points trial users to web quiz before LINE', () => {
  assert.match(html, /還沒準備購買？免登入先練習，答完再看整理/);
  assert.match(html, /免登入開始練習/);
  assert.match(html, /免登入練習/);
  ...
  assert.doesNotMatch(html, /先免費做題|免費做題，看弱點/);
...
test('serial and magic login mark local study progress for dashboard handoff', () => {
  ...
  assert.match(html, /localStorage\.setItem\('sofa_uid', data\.uid\);[\s\S]{0,260}sofaTrack\('serial_verify_success', \{ method: 'magic'[\s\S]{0,180}markStudyLocalProgressForLogin\(\);[\s\S]{0,180}window\.location\.href = dashboardTargetAfterLogin\(\)/);
  assert.match(html, /if \(data\.token\) localStorage\.setItem\('sofa_token', data\.token\);[\s\S]{0,260}sofaTrack\('serial_verify_success', \{ method: 'serial'[\s\S]{0,180}markStudyLocalProgressForLogin\(\);[\s\S]{0,180}window\.location\.href = dashboardTargetAfterLogin\(\)/);
});

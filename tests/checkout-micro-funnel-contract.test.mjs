...
test('checkout records pre-payment visibility and selection micro-funnel events', () => {
  ...
  assert.match(checkout, /function observeCheckoutSignal\(el, eventName, options\)/);
  assert.match(checkout, /const checkGeometry = \(\) => \{/);
...
test('checkout visibility events happen before checkout start is counted as a page-load baseline', () => {
  assert.ok(
    checkout.indexOf('observeCheckoutSignal(document.querySelector(".email-card"), "checkout_form_visible"') <
      checkout.indexOf('trackCheckout("checkout_start")'),
    'visible email/payment form should be registered before the page-load checkout_start baseline'
  );
  assert.ok(

...
test('study playlist is a generic text fallback and does not ship private schedules', () => {
  ...
  assert.match(active, /\/api\/playlist\?/);
  assert.match(active, /params\.set\('mode', 'active_recall'\)/);
  assert.match(active, /params\.set\('content_layer', 'analysis'\)/);
  assert.match(active, /params\.set\('pause_seconds', pauseSeconds \|\| '6'\)/);
  assert.match(active, /function studyPlaylistApiUrl/);
  assert.match(active, /params\.set\('star_min', String\(starMin \|\| '5'\)\)/);
...
test('study playlist items are executable while article reading stays in the dashboard', () => {
  ...
  assert.match(fn, /var practiceHref = 'quiz\.html\?' \+ practiceParams\.join\('&'\)/);
  assert.match(fn, /practiceParams\.push\('drill=1'\)/);
  assert.match(fn, /item\.page_id \|\| item\.id/);
  assert.match(fn, /practiceParams\.push\('page_id=' \+ encodeURIComponent\(itemId\)\)/);
...
test('study tool panels expose one active mode and explain where saved work goes', () => {
  ...
  assert.match(active, /typeof updateStudyPlaylistToggleLabel === 'function'/);
...
test('study today action buttons are sized for mobile app shells', () => {
  ...
  assert.match(actionBlock, /class="study-action-group secondary"[\s\S]*<span class="study-action-label">安排<\/span>[\s\S]*播放清單[\s\S]*排課[\s\S]*補紀錄/);
  assert.match(actionBlock, /播放清單/);
  assert.doesNotMatch(actionBlock, /重點朗讀|聽讀清單/);
  assert.doesNotMatch(active, /看時間、弱點和下一堂；要排課再開下方工具/);
...
test('study plan items expose learning actions only when item metadata supports them', () => {
  ...
  assert.match(helper, /var quizHref = 'quiz\.html\?' \+ quizParams\.join\('&'\)/);
  assert.match(helper, /quizParams\.push\('page_id=' \+ encodeURIComponent\(pageId\)\)/);
...
test('sidebar and mobile quick entry use clear exam-loop labels', () => {
  ...
  assert.match(active, /<a href="#study-playlist-block" data-spy-target="study-playlist-block"><span class="num">T5<\/span>播放清單/);

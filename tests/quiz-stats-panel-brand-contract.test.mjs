...
test('stats overlay owns the iOS safe area instead of leaving a white strip', () => {
  ...
  assert.match(active, /\.stats-panel\{[\s\S]*background:[\s\S]*radial-gradient/);
  assert.match(active, /\.stats-panel\{[\s\S]*var\(--navy\)/);
  ...
  assert.doesNotMatch(active, /\.stats-panel\{[\s\S]*background:#040D14/);
